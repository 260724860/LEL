...
namespace LELAdmin.Controllers
{
    ...
    public class UploadExcelController : ApiController
    {
        ...
        /// <returns></returns>
        [AllowAnonymous]
        [HttpPost, Route("api/UploadExcel/ExportGoodsPriceTemplate/")]
        public IHttpActionResult ExportGoodsPriceTemplate()
        {

            return Json(JRpcHelper.AjaxResult(0, "SUCCESS", "/UploadFile/爆品改价.xlsx"));

            //try
            //{
            //    DataTable GoodsSource = new DataTable();
            //    DataTable Goods
            //    string path = System.Web.HttpContext.Current.Server.MapPath("/") + "UploadFile/爆品改价.xlsx";
            //    string FileName = RandomUtils.GenerateOutTradeNo("Excel") + ".xlsx";
            //    string SavPath = System.Web.HttpContext.Current.Server.MapPath("/") + "UploadFile/export/" + FileName;
        ...
        /// <returns></returns>
        [AllowAnonymous]
        [HttpPost, Route("api/UploadExcel/ImportGoodsSpecialOffer/")]
        public IHttpActionResult ImportGoodsSpecialOffer()
        {
            System.Web.HttpContext context = System.Web.HttpContext.Current;
            //string serverPath = System.Configuration.ConfigurationSettings.AppSettings["serverPath"];
            //string localPath = System.Configuration.ConfigurationSettings.AppSettings["localPath"];
            ...
            if (cout > 0)
            {
                ...
                if (hpf != null)
                {
                    ...
                    string fileName = RandomUtils.GenerateOutTradeNo("爆品改价") + Path.GetExtension(hpf.FileName);

                    string filePath = System.Web.HttpContext.Current.Server.MapPath("/") + folderPath + fileName;
                    hpf.SaveAs(filePath);
                    // string path = System.Web.HttpContext.Current.Server.MapPath("/") + "UploadFile/1.xlsx";
                    var ImportData = UploadFileBLL.UpdateGoodsBySpecialOfferExcel(filePath, out string Msg);
                    if (ImportData)
                    {
                        return Json(JRpcHelper.AjaxResult(0, "上传成功", null));
                    }
        ...
        [AllowAnonymous]
        [HttpPost, Route("api/UploadExcel/DeleteGoodsByExcel/")]
        public IHttpActionResult DeleteGoodsByExcel(string FileName)
        {
            string path = System.Web.HttpContext.Current.Server.MapPath("/") + "UploadFile/" + FileName;// + ".xls";
                                                                                                        //try
            try
            {
                var result = new UploadFileService().DeleteGoodsByExcel(path, out string Msg);
                return Json(JRpcHelper.AjaxResult(0, Msg, null));
            }
            catch(Exception ex)
            {
